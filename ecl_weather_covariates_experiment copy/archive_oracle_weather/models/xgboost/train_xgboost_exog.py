"""
XGBoost exogenous baseline for the independent ECL + Lisbon weather experiment.

Reference pattern: dissertation/models/xgboost/train_xgboost.py

Changes from the load-only baseline:
- Reads outputs/electricity_lisbon_weather.csv.
- Uses physical_datetime for chronological split and calendar alignment.
- Preserves benchmark_datetime only as metadata; it is never a model feature.
- Adds Lisbon weather/calendar covariates to lag and rolling load features.
"""
...
RESULTS_CSV = RAW_RESULTS_DIR / "xgboost_exog_oracle_weather_results.csv"
FEATURE_IMPORTANCE_CSV = RAW_RESULTS_DIR / "xgboost_exog_oracle_weather_feature_importance.csv"
...
def evaluate_client(client_id: str, split, client_idx: int, total_clients: int) -> tuple[list[dict], list[dict]]:
    ...
    test_exog_scaled = exog_scaler.transform(test_exog)

    train_features = build_feature_frame(
        train_scaled,
        physical_index(split.train),
        train_exog_scaled,
        split.exog_columns,
    )
    ...
    if train_features.empty or val_features.empty:
        print(f"Skipping client {client_id}: insufficient rows after lag feature construction.")
        return []
    ...
    feature_importance_records = [
        {
            ...
            "model": "xgboost_exog_oracle_weather",
    ...
    for horizon in HORIZONS:
        ...
        # Fixed-origin evaluation: every horizon starts from the same
        # train+val boundary, matching the dissertation baseline scripts.
        history_scaled = list(np.concatenate([train_scaled, val_scaled]))
        preds_scaled = []
        t1 = time.time()
        for step in range(horizon):
            next_row = build_next_feature_row(
                history_scaled,
                test_exog_scaled[step],
                split.exog_columns,
            )
            pred_scaled = float(model.predict(next_row[feature_cols].values.astype(np.float32))[0])
...
def main() -> None:
    ...
    if args.dry_run:
        dry_run_report(split, "XGBoost exogenous oracle-weather")
        return
